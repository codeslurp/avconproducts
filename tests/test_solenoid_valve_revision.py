...
"Solenoid Valve_Updated4.xlsx" (2026-09-12) supersedes _Updated3. So plain
...
Note the row/code asymmetry: 89 rows but 88 distinct codes. SR432A3RWK02 is
used for two genuinely different products (BSP vs NPT end connection). That
defect predates this drop — it is present in _Updated2 and _Updated3 too —
and is recorded in
docs/engineering-followups/2026-09-12-solenoid-valve-updated4.md.
"""
import os
import sys
import unittest
...
# a silent rollback to _Updated3 would still fail the new-code assertions.
NEW_CODES = [
    # added by the 2026-07-29 revision (still in _Updated4)
...
class TestSolenoidRevision(unittest.TestCase):
    ...
    def test_distinct_code_count(self):
        # 88 distinct across 89 rows — see the SR432A3RWK02 note in the docstring.
        codes = [r["code"] for r in self.rows]
        self.assertEqual(len(set(codes)), 88)

    def test_family_count_matches_rows(self):
        fams = {f["key"]: f for f in self.loaded["families"]}
        self.assertEqual(fams["Solenoid Valve"]["count"], 89)

    def test_source_is_updated4(self):
        # Extra-source solenoid is not the consolidated workbook; confirm the
        # family actually came from the newest dedicated file via a unique code.
        codes = {r["code"] for r in self.rows}
        self.assertIn("SR123C3SWK02", codes)
